...
void i2c1_manager::i2cWrite(uint8_t registerAddress, uint8_t length, uint8_t address) {
	...
	uint8_t rcode = Wire1.endTransmission(I2C_MANAGER_RELEASEBUS);
	if (rcode) {
		error = 1;
		error_counter++;
	}
...
void i2c1_manager::i2cRead(uint8_t registerAddress, uint8_t nbytes, uint8_t address) {
	...
	Wire1.requestFrom(address, nbytes, (uint8_t)I2C_MANAGER_RELEASEBUS);
...
void i2c1_manager::i2c_init() {
	Wire1.begin();
	Wire1.setClock(I2C_MANAGER_CLOCK);
	error = 0;
	error_counter = 0;
}
